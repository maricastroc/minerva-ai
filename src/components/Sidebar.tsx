        ...
        <>
          <div className="p-6 pt-3 flex items-center justify-between w-full pb-2">
            <Image
              ...
              height={140}
              alt="Logo"
              src={isDarkTheme ? '/logo-full.svg' : '/logo-full_light.svg'}
            />
            <button
              onClick={() => setIsOpen(!isOpen)}
              className="cursor-pointer mt-[0.2rem] bg-transparent p-2 mr-[-0.7rem] hover:bg-gray-600 rounded-md transition-colors"
            >
          ...
          <UserSection mutateChats={mutate} />
        </>
      ) : (
        <button
          onClick={() => setIsOpen(true)}
          className="cursor-pointer p-4 bg-transparent rounded-md hover:bg-gray-700 transition-colors"
        >
